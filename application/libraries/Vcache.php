...
// ------------------------------------------------------------------------
   ...
   * @return mixed
   */
  public function get ($key,$return_as_array=FALSE,$id_user=NULL) {
    $key   = $this->_secure_key($key);
    $where = array($this->_db_cache_key=>$key);
    if($id_user) $where['id_user'] = $id_user;
    $cont  = $this->_ci->db->get_where($this->_db_tbl_name,$where);
    if($cached = $cont->row()) {
      if($cached->expires_at) {
        ...
        if($now >= $expire) {
          ...
          return json_decode($cached->$cached_val_field,$return_as_array);
        }
      } else {
        $cached_val_field = $this->_db_cache_val;
        return json_decode($cached->$cached_val_field,$return_as_array);
